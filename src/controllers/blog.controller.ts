...
export const addLikeToPost = async (req: Request, res: Response) => {
  ...
  const blogObj = await DaBlog.update(
    ...
    {
      $addToSet: { likes: { likedBy: userObj._id, likedByName: userObj.name } },
      $inc: { totalLikes: 1 },
    }
  );
...
export const getLandingPageBlog = async (req: Request, res: Response) => {
  let { categoryId, page, pageSize , onlyApproved, mostLikedFirst, mostCommentedFirst } = req.body;
  page = page ? page : 1;
  ...
  const blogsObj = await DaBlog.readAll({ filter, page, pageSize, populate , orderBy});
